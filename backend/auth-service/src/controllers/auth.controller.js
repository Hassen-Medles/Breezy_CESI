...
function signToken(payload) {
  return jwt.sign(payload, process.env.AUTH_TOKEN, { expiresIn: "1h" });
}
...
export const register = async (req, res) => {
  try {
    ...
    res.status(201).json({ message: "Vérifiez votre email." });
  } catch (err) {
...
export const getProfile = async (req, res) => {
  try {
    // On retourne les infos du user extraites du token (id, email)
